                   ...
                   segmentLabels:(NSDictionary<NSString *, NSString *> *)segmentLabels
{
    ...
    CSStreamSense *streamSense = [SRGAnalyticsTracker sharedTracker].streamSense;
    [[streamSense labels] removeAllObjects];
    [labels enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSString * _Nonnull object, BOOL * _Nonnull stop) {
        [streamSense setLabel:key value:object];
    }];
    ...
    [[[streamSense clip] labels] removeAllObjects];
    [segmentLabels enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSString * _Nonnull object, BOOL * _Nonnull stop) {
        [[streamSense clip] setLabel:key value:object];
    }];
    
    [streamSense notify:eventType.intValue position:position labels:nil /* already set on the stream and clip objects */];
}
